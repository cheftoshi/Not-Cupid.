-- Public launch record for the first Boston Dating Experiment. The operator
-- completed the iPhone/PWA walkthrough and approved a single 400-person entry
-- cap on August 15, 2026. The winner limit remains two pairs.
...
begin
  ...
  select status, terms_version into v_existing_status, v_existing_terms_version
  from public.raffle_entries
  where user_id = p_user_id and event_key = p_event_key
  for update;
  v_was_new := not found
    or v_existing_status = 'withdrawn'
    or v_existing_terms_version is distinct from v_event.terms_version;
  ...
  if v_was_new then
    ...
    from public.raffle_entries
    where event_key = p_event_key
      and terms_version = v_event.terms_version
  ...
  if v_existing_status is null then
    ...
    update public.raffle_entries set
      ...
      attempts = case
        when status = 'withdrawn' or terms_version is distinct from p_terms_version then 0
        else attempts
      end,
...
begin
  ...
  set entry_cap = 400,
      terms_version = 'boston-v13-2026-08-15',
      operator_compliance_approved = true,
      operator_compliance_approved_at = now(),
      operator_compliance_reference = 'NotCupid operator approval of 400-entry public-launch boston-v13-2026-08-15 after completed iPhone/PWA walkthrough on 2026-08-15',
      status = 'entry_open',
      updated_at = now()
  where event_key = 'boston-dating-experiment-v1'
    and status = 'entry_open'
    and entry_cap = 100
    and terms_version = 'boston-v12-2026-08-15'
    and prize_funding_confirmed
    and venue_confirmed
    and sponsor_details_confirmed
    and operator_compliance_approved;

  if not found then
    raise exception 'Public-launch v13 requires the fully approved v12 rehearsal event at capacity 100';
  end if;
end
$$;
