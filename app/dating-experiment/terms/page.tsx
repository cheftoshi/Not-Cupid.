...
export default async function DatingExperimentTermsPage() {
  ...
  return (
    <LegalPage title="Dating Experiment Official Rules & Terms" subtitle="No purchase necessary. The complete rules for Dinner on Us: Boston." updated="August 15, 2026">
      ...
      <h2>1. Free entry</h2>
      <p>Entry is free through the Dating Experiment screen. No purchase or payment is necessary to enter, receive a shortlist, or be selected. A purchase, subscription, donation, extra Love connection, or NotCupid Pro membership does not add entries, change shortlist priority, or improve selection odds. Limit one entry per person.</p>
      ...
      <p>Use the Dating Experiment screen, choose how you identify, choose the orientation label that feels closest, select one or more genders you would be open to meeting, choose an inclusive minimum and maximum age from 21 through 99, select every listed dinner time you can actually attend, answer four short experiment prompts about intent, dinner energy, planning style, and a conversation starter, optionally submit an original {RAFFLE.videoMinSeconds}–{RAFFLE.videoMaxSeconds}-second introduction video, and accept the required consent notices. Choosing not to add a video does not affect eligibility, fit score, shortlist priority, or selection odds. These preferences and time availability are saved with the entry, do not alter the entrant&apos;s general Love Line settings, and control this experiment even if the entrant later edits their general NotCupid profile. The orientation label is shown only to private shortlist participants; selecting &quot;prefer not to label&quot; displays no orientation label. Entries that are automated, duplicated, incomplete, misleading, abusive, or use content the entrant does not have the right to share may be disqualified.</p>
      ...
      <h2>7. Dinner</h2>
      <p>Up to {RAFFLE.winnerPairCount} dinner prizes are available, one for each selected pair: one reservation on Thursday, August 20, 2026, at 6:30 PM Eastern Time and one at 8:30 PM Eastern Time. The restaurant and final slot assignment will be shared privately after selection. Each prize is one prepaid dinner at a selected Boston restaurant with a maximum approximate retail value of ${RAFFLE.budget} per pair. Food, alcoholic or non-alcoholic drinks, ordinary tax, and gratuity may all count toward that same cap. Selected participants do not pay or request reimbursement for the included dinner, but are responsible for any amount above the prepaid ${RAFFLE.budget}, plus transportation, parking, or valet charges. The maximum aggregate value of all prizes is ${RAFFLE.budget * RAFFLE.winnerPairCount}. A prize has no cash alternative, is not transferable, and may be rescheduled or substituted only with an equal-or-greater-value experience if the venue becomes unavailable or circumstances make fulfillment impracticable.</p>
