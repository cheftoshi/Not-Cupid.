...
export default function DatingExperimentTermsPage() {
  return (
    <LegalPage title="Dating Experiment Official Rules & Terms" subtitle="No purchase necessary. The complete rules for Dinner on Us: Boston." updated="August 8, 2026">
      ...
      <h2>2. Eligibility</h2>
      <p>You must be 21 or older, reside in Massachusetts within approximately {RAFFLE.radiusMiles} miles of ZIP {RAFFLE.centerZip}, hold one genuine NotCupid account, complete the required profile and intro-video steps, and be able to attend the stated Boston dinner. Employees, contractors, and members of their immediate households are not eligible. Test, duplicate, automated, and fraudulent accounts are excluded. The experiment supports reciprocal preferences involving men, women, and non-binary / another-identity participants, including different-gender and same-gender pairings; no pairing is guaranteed. Void where prohibited.</p>
      ...
      <p>Use the Dating Experiment screen, choose how you identify, select one or more genders you would be open to meeting, choose an inclusive minimum and maximum age from 21 through 99, confirm availability, answer the short experiment questions, submit an original {RAFFLE.videoMinSeconds}–{RAFFLE.videoMaxSeconds}-second introduction video, and accept the required consent notices. These preferences are saved with the entry, do not alter the entrant&apos;s general Love Line settings, and control this experiment even if the entrant later edits their general NotCupid profile. Entries that are automated, duplicated, incomplete, misleading, abusive, or use content the entrant does not have the right to share may be disqualified.</p>

      <h2>5. Compatibility and selection</h2>
      <p>The system first applies mutual age and gender preferences, location eligibility, prior-pair exclusions, and a minimum compatibility threshold. A pair can be considered only when each person&apos;s selected genders include the other person&apos;s identity and each person&apos;s age falls inside the other person&apos;s inclusive age range. Compatibility is calculated primarily from NotCupid&apos;s values, attachment, personality, and lifestyle model, with smaller contributions from shared interests and the experiment questions.</p>
