...
export default function DatingExperimentTermsPage() {
  return (
    <LegalPage title="Dating Experiment Official Rules & Terms" subtitle="No purchase necessary. The complete rules for Dinner on Us: Boston." updated="August 8, 2026">
      ...
      <p>The entry period opens only when the Dating Experiment screen states that entries are open and closes at {RAFFLE.entryCloseLabel}, or when the published cap of {RAFFLE.cap} eligible entries is reached, whichever occurs first. NotCupid&apos;s server clock is the official timekeeper. The intended dinner dates are August 19 and August 21, 2026. Exact times, restaurant details, and final pair-to-date assignments will be confirmed later. Entries remain closed while the entry and shortlist deadlines are TBD.</p>

      <h2>4. How to enter</h2>
      <p>Use the Dating Experiment screen, choose how you identify, choose the orientation label that feels closest, select one or more genders you would be open to meeting, choose an inclusive minimum and maximum age from 21 through 99, confirm you can attend at least one listed dinner date, answer the short experiment questions, submit an original {RAFFLE.videoMinSeconds}–{RAFFLE.videoMaxSeconds}-second introduction video, and accept the required consent notices. These preferences are saved with the entry, do not alter the entrant&apos;s general Love Line settings, and control this experiment even if the entrant later edits their general NotCupid profile. The orientation label is shown only to private shortlist participants; selecting &quot;prefer not to label&quot; displays no orientation label. Entries that are automated, duplicated, incomplete, misleading, abusive, or use content the entrant does not have the right to share may be disqualified.</p>

      <h2>5. Compatibility and selection</h2>
      <p>The system first applies mutual age and gender preferences, location eligibility, prior-pair exclusions, and a minimum compatibility threshold. A pair can be considered only when each person&apos;s selected genders include the other person&apos;s identity and each person&apos;s age falls inside the other person&apos;s inclusive age range. The orientation label is self-described context, not a substitute for the selected-gender rules, and does not independently add or remove a candidate. Compatibility is calculated primarily from NotCupid&apos;s values, attachment, personality, and lifestyle model, with smaller contributions from shared interests and the experiment questions.</p>
      ...
      <p style={{ marginTop: '1.5rem', color: 'var(--h-text-dim)', fontSize: '0.85rem' }}>These Official Rules are a working legal document for a recreational project and must receive Massachusetts counsel review, along with confirmation of Sponsor details, entry and shortlist deadlines, exact dinner times, venue, and prize funding, before entries open. Contact <a href={`mailto:${CONTACT}`}>{CONTACT}</a> for a copy or question.</p>
    </LegalPage>
  );
}
