...
const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: 'A live event series — Boston first. Entering is free, and each round raffles one fully-covered date (up to $200 at a Boston restaurant). It is a real raffle — luck — but the odds scale with compatibility: every eligible pair can be drawn, and better-matched pairs (weighted toward shared interests — hobbies, music, food, sports — on top of personality and values) win more often. To keep it fair we balance the pool by gender so it can\'t skew lopsided, and anyone who already won a past round sits out the next. The rules: you must be in Boston with a complete profile to enter; each round is capped at 100 entrants and entry closes a few days before; the draw is fully automatic — no human picks — the moment we hit 100 or at the deadline. When you\'re drawn you get two choices: accept or reject. Both people have to accept for the date to lock in. If someone rejects, the other person gets re-drawn to another match — everyone gets up to two shots, then they\'re set aside for the round. A short intro video is required to enter (your contest intro), and you can turn on notifications so we ping you the second you\'re picked. NotCupid Pro members get double entry (2× the odds); no purchase is necessary, though — the same bonus entry is free on request. No purchase necessary; open to Boston-area residents 21+; winner selected by chance. Full terms are in the Official Rules (notcupid.com/raffle/rules).',
  },
  {
