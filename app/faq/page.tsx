...
const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: 'A live event series — Boston first. Entering is free, and each round raffles one fully-covered date (up to $200 at a Boston restaurant). Instead of a random pull, our matching algorithm draws the single most compatible pair from the entrants — weighted heavily toward shared interests (hobbies, music, food, sports) on top of personality and values. The rules: you must be in Boston with a finished profile (photo + quiz) to enter; each round is capped at 100 entrants and entry closes a few days before; the draw is fully automatic — no human picks — the moment we hit 100 or at the deadline. When you\'re drawn you get two choices: accept or reject. Both people have to accept for the date to lock in. If someone rejects, the other person gets re-drawn to the next best match — everyone gets up to two shots, then they\'re set aside for the round. A short intro video is required to enter (your contest intro), and you can turn on notifications so we ping you the second you\'re picked.',
  },
  {
