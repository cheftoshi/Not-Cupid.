...
// Draw the raffle: pair eligible entrants by the hobbies-weighted raffleScore
// (greedy, non-overlapping), create pending draws, notify both sides.
export async function POST() {
  ...
  if (ids.length < 2) return NextResponse.json({ ok: true, drawn: 0, message: 'Need at least 2 fresh entrants to draw.' });

  const { data: usersData } = await supabaseAdmin.from('users').select(COLS).in('id', ids);
  const users: any[] = (usersData as any[]) ?? [];
  const pool: any[] = users.filter((u: any) => u.photo_url && u.archetype && metroOf(u.zip) === RAFFLE.metro);

  // Don't re-draw anyone already in a live draw.
  ...
  const avail = pool.filter((u: any) => !taken.has(u.id));

  // All mutually-eligible pairs, scored.
  ...
  // Greedy: best pair first, no one drawn twice.
  ...
  return NextResponse.json({ ok: true, entrants: pool.length, drawn: drawn.length, pairs: drawn });
}
