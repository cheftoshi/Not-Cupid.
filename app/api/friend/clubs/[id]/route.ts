...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!c || c.hidden_at || (c.is_test === true) !== ((user as any).is_test === true) || (metroOf((user as any).zip) && c.metro !== metroOf((user as any).zip))) {
  ...
  const viewerMetro = metroOf((user as any).zip);
  const visibleUsers = (users ?? []).filter((member: any) =>
    (member.is_test === true) === ((user as any).is_test === true) && (!viewerMetro || metroOf(member.zip) === viewerMetro)
  );
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'approve' || action === 'decline') {
    ...
    if (action === 'approve') {
      ...
      if (!candidate || (candidate.is_test === true) !== ((user as any).is_test === true) || (metroOf((user as any).zip) && metroOf(candidate.zip) !== metroOf((user as any).zip))) {
