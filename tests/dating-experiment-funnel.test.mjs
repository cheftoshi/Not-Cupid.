...
test('campaign profile completion hands eligible people directly into the Experiment', () => {
  ...
  assert.match(destinations, /profile: '\/profile\?mode=edit&from=dating-experiment-comeback'/);
  assert.match(profilePage, /experimentMode=\{params\.from === 'dating-experiment-comeback'\}/);
  assert.match(profileForm, /event: 'profile_started'/);
  assert.match(profileForm, /X-NotCupid-Funnel/);
  assert.match(profileShell, /\/dating-experiment\?from=profile-complete/);
...
test('campaign funnel ledger is private, idempotent, backfilled, and visible as aggregate admin stages', () => {
  ...
  assert.match(experiment, /event: 'experiment_viewed'/);
