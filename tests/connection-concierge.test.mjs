...
test('Hub concierge is consented, bounded, measurable, and phone-first', () => {
  ...
  assert.match(css, /height: calc\(var\(--app-visual-viewport-height, 100svh\) - var\(--app-top-nav-height/);
  ...
  assert.match(client, /showControls &&/);
  assert.doesNotMatch(client, /className=\{styles.conciergeMatchingInvite\}/);
