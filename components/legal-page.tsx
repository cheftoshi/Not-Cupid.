...
export default function LegalPage({ title, subtitle, updated, children }: {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--h-bg)', color: 'var(--h-text)' }}>
      ...
      <main style={{ boxSizing: 'border-box', flex: 1, minWidth: 0, maxWidth: 760, margin: '0 auto', padding: '2.5rem calc(1.25rem + env(safe-area-inset-right, 0px)) calc(1rem + env(safe-area-inset-bottom, 0px)) calc(1.25rem + env(safe-area-inset-left, 0px))', width: '100%' }}>
        ...
        <style>{`
          ...
          .legalBody p { margin: 0 0 0.9rem; overflow-wrap: anywhere; }
          .legalBody ul { margin: 0 0 1rem; padding-left: 1.2rem; }
          .legalBody li { margin: 0 0 0.4rem; }
          .legalBody a { color: #1b46c9; overflow-wrap: anywhere; }
